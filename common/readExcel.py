...
class ReadExcel():
    ...
    #     global excel_data, read_file, table, rownum, colnum
    #     read_file = xlrd.open_workbook(file_url, 'r')
    #     table = read_file.sheet_by_name(sheetname)
    #     rownum = table.nrows
    #     colnum = table.ncols
    #     excel_data = []
    #     for i in range(rownum):    # 把每行作为一个元素放到excel_data这个列表里面
    #         excel_data.append(table.row_values(i))
    #
    ...
    #             for j in range(para_num):
    #                 row_data.append(excel_data[i][dict[filter_para[j]]])
    #             final_data.append(row_data)
    #         else:
    #             continue
    #
